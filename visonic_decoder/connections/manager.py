...
class ConnectionProfile:
    ...
    forward_messages_to: ConnectionName | None = None
...
class ConnectionManager:
    ...
    def _handle_message(self, source: ConnectionName, client_id: str, msg: bytes):
        ...
        destination = profile.forward_messages_to

        # Handle special commands
        decoded_msg = msg.decode("ascii", errors="ignore")
        if decoded_msg in [c.name for c in list(Commands)]:
            self._cb_received_message(source, client_id, "COMMAND", decoded_msg)
            # self._loop.create_task(self.shutdown())
        else:
            self._cb_received_message(source, client_id, destination, msg)
